...
La experimentación se cierra con las 30 corridas realizadas. Se cancela la ejecución adicional de E10, E11 y las repeticiones propuestas. Las gráficas proceden de registros reales y se conservan los resultados originales. El cierre se basa en analizar esos resultados y decidir las medidas de arquitectura, sin generar nuevas cargas.
...
Una captura permite ver una observación; los archivos por corrida permiten comprobar el resultado. El paquete contiene configuración, resultados originales, resultados recalculados y huellas de integridad. Los registros originales de las 30 corridas se publican por separado en los ZIP de la [versión laboratorio-s7](https://github.com/AlejandroForeroG/solventa-laboratorio/releases/tag/laboratorio-s7). La [guía de reproducción](reproduccion.md) explica cómo verificarlos y recalcularlos.
...
E10 estaba previsto para comparar plazos de 80, 120 y 160 ms; E11, para confirmar la opción seleccionada. No se ejecutaron y se cancela su ejecución adicional. El cierre mantiene los 120 ms utilizados en los laboratorios, coherentes con el presupuesto por dependencia del caso Solventa, sin afirmar que sean un óptimo entre todas las alternativas.

La conclusión de esta entrega se fundamenta en los resultados de E01 a E09. Los ajustes de arquitectura responden a los incumplimientos observados. Los registros de la campaña y sus estados automáticos se conservan como evidencia del trabajo realizado.
...
El cálculo de sondeos está corregido y verificado. Para el producto se mantendrá un identificador de extremo a extremo y un registro durable del resultado de negocio en Acquisition & Risk. El servicio responsable guardará el estado de la operación y su evento de auditoría en una misma transacción. El patrón outbox conserva los eventos pendientes de publicación. Estos se procesarán de forma asíncrona e idempotente, es decir, sin duplicar sus efectos si se reciben más de una vez. Se registrará por separado el despacho y el recibo del proveedor: un despacho sin confirmación seguirá identificado como tal. Así podremos reconstruir la decisión aunque falten eventos en el panel de observabilidad. El cambio requiere almacenar y conciliar esos registros.
...
Se propone incorporar la recuperación acotada ya desarrollada como opción diagnóstica: primer intento de SELECT de hasta 200 ms, cierre de la conexión y como máximo un reintento ante un error transitorio permitido, dentro del límite total de 700 ms. El límite de reintentos por instancia y dependencia evita multiplicar las consultas; no se reintentan escrituras ni errores de autorización. Si el consentimiento no puede verificarse, el acceso continúa bloqueado. Si falla la lectura del respaldo después de autorizar, se propone devolver un estado preliminar sin datos ni oferta y conservar el error técnico. Las pruebas diagnósticas comprobaron esos comportamientos y registraron respuestas de hasta 836 ms. Proponemos incorporar esta recuperación, aunque su efecto sobre el p99 todavía no se ha medido.

### 4.4. Recuperación y alcance del circuito

Se conserva el circuito por instancia y operación: ventana de 20 muestras en 30 s, mínimo diez, apertura al 50 % de fallas, espera de 30 s, máximo dos sondeos por ventana y cierre tras cinco éxitos. La reducción de llamadas de E05 y los 219 cierres de E06 respaldan esta elección. El seguimiento del producto distinguirá los estados abierto, en recuperación, cerrado y sin actividad, junto con la última solicitud observada. Un proceso que deja de recibir tráfico no se tratará automáticamente como un circuito atascado. No se añade coordinación global al recorrido de respuesta.

### 4.5. Incorporación de las medidas al producto

Cada componente conservará su responsabilidad: Identity verifica consentimiento; Acquisition & Risk obtiene señales, responde y mantiene su respaldo y auditoría; Queue transporta actualizaciones con deduplicación; CockroachDB guarda los datos de cada servicio. Las propuestas se registrarán como decisiones de arquitectura y tareas de implementación. No se sustituye la consulta de consentimiento por una copia permisiva ni se agregan reintentos en cadena al proveedor. La cotización preliminar se identifica como tal y la oferta definitiva requiere datos elegibles y cálculo completo.
...
Los criterios proceden del [protocolo experimental preregistrado de Semana 5](https://docs.google.com/document/d/1ojgu0vQBPDDUtLLbq0zzLS3dnSZHvwO2PkfPbBGdzNM/edit), cuya copia se conserva en [protocolo-s5.md](protocolo-s5.md). La [entrega oficial de Semana 7](https://www.coursera.org/learn/miso-proyecto-1/assignment-submission/J2Z0o/entrega-semana-7) y su matriz se documentan en 00-requisitos-y-control.md.

La [carpeta de evidencias experimentales](https://drive.google.com/drive/folders/11Mw4NfLJkKveZMJeMDr9IgAThBlqIuGO) contiene copias de resultados, configuración y manifiestos; analisis-30.json incluye métricas, incidencias, eventos, series, comparaciones y 120 huellas de archivos fuente. verificacion-ventanas.json conserva configuración emparejada, aperturas y grupos censurados. Se entregan seis CSV para revisión, nueve gráficas en PNG/PDF y cinco capturas originales, enlazados en 02-registro-evidencias.md.

La [guía de reproducción](reproduccion.md) permite recalcular E05 r2 con los registros incluidos, o las 30 corridas con los ZIP de la versión laboratorio-s7. El manifiesto conserva huellas de los archivos y de cada ZIP. Los CSV, gráficas y análisis derivados se entregan en resultados/. Los scripts de análisis agregado se conservan en tools/analisis/.
